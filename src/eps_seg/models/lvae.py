...
class LVAEModel(L.LightningModule):
    ...
    def log_step(self, 
        ...
        if outputs.get("pseudo_labels_stats") is not None and segments is not None:
            ...
            for l, layer_probs in enumerate(pl_stats["pseudo_labels_confidences"]):
                # How confident each layer is in assigning pseudo-labels to neighbors in general
                # TODO: this is not only for neighbors!
                conf = layer_probs.max(dim=-1).values if layer_probs.ndim > 1 else layer_probs
