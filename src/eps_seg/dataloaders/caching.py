...
class DatasetCache:
    ...
    def build_fold_assignments(self, substacks: List[dict]) -> List[dict]:
        ...
        For max_folds > 1, StratifiedKFold is applied to shuffled substacks using
        the source stack as the stratum label. For max_folds == 1, a deterministic
        shuffled split is created within each source stack.
        """
        rng = random.Random(self.cfg.seed)
    ...
    def build_trainval_cache_payload(self) -> Dict[str, object]:
        """
        Build the canonical-anchor training/validation payload from the original
        dataset without relying on a pre-existing cache.

        A payload is a dictionary containing all the data structures required to write the shared
        cache files and materialize any fold's train/validation datasets. The payload is built
        once and then passed to the write_cached_dataset_splits method to persist the cache.
        """
    ...
    def load_uncached_trainval_fold(self, fold: int) -> Dict[str, object]:
        ...
        without writing any cache files.
        """
        payload = self.build_trainval_cache_payload()
