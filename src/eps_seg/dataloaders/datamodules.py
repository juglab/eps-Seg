...
class BetaSegDataModule(L.LightningDataModule):
    ...
    def _cache_dataset_splits(self):
        ...
        assert Path(self.cfg.cache_dir).resolve() != Path(self.cfg.data_dir).resolve(), "cache_dir and data_dir must not be the same."
    ...
    def _load_cached_dataset_splits(self):
        if not Path(self.cfg.cache_dir).resolve().exists():
            raise FileNotFoundError(f"Cache directory {self.cfg.cache_dir} does not exist.")
        # Load cached splits, mean, std from cache_dir
        self.train_idx = np.load(Path(self.cfg.cache_dir) / "train_idx.npy", allow_pickle=True).item()
        self.val_idx = np.load(Path(self.cfg.cache_dir) / "val_idx.npy", allow_pickle=True).item()
    ...
    def _compute_statistics(self, images: Dict[str, np.ndarray], train_idx: Dict[str, np.ndarray]):
        """
    ...
    def _normalize_data_inplace(self, images: Dict[str, np.ndarray], data_mean: float, data_std: float) -> Dict[str, np.ndarray]:
        """
