...
class TrainConfig(BaseEPSConfig):
    ...
    early_stopping_patience: int = Field(default=120, description="Patience for early stopping")
    ...
    initial_threshold: float = Field(default=0.70, description="Initial confidence threshold for training in semisupervised mode")
    max_threshold: float = Field(default=0.99, description="Maximum confidence threshold for training in semisupervised mode")
    threshold_increment: float = Field(default=0.0005, description="Step size for confidence threshold increase")
