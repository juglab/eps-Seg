...
class BetaSegDatasetConfig(BaseEPSDatasetConfig):
    ...
    test_center_slice: int = Field(626, description="Center slice index for testing dataset")
    test_stepping: int = Field(3, description="Stepping value for slicing the test dataset (applies in all dimensions, only for testing)")
    test_half_depth: int = Field(6, description="Half depth of the test volume to consider around the center slice (applies in Z dimension for testing)")
    predict_center_slice: int = Field(626, description="Center slice index for prediction dataset")
    predict_half_depth: int = Field(0, description="Half depth of the prediction volume to consider around the center slice (applies in Z dimension for prediction)")
